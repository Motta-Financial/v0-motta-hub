...
| Operation | Documented | Built | Notes |
|---|---|---|---|
| `POST /v2/clients/oii-client/{clientOiiId}/returns` (create return) | ⚠️ Not in the authoritative Phase 1 v3 doc — only in the original doc | ✅ **CONFIRMED LIVE 2026-08-24** | First successful call ever: HTTP **201** on client `9341…9628` (TY2025 IND), intuit-tid `1-6a8ef82e-…`, via `scripts/403-verify-create-tax-return.ts`. The inferred host and path were correct. **201 returns an EMPTY BODY** — no engagement id, so the created return has to be resolved by re-syncing that client's engagements for the year and matching on the requested name. Leadership-gated and audit-logged. Still no delete endpoint. |
