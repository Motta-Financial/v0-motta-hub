...
export const ALLOWED_TABLES = [
  ...
  "pipeline_stages",
  "pipelines",
  // NOTE: proconnect_clients is deliberately NOT allow-listed — its
  // tax_id column carries SSNs/EINs. Use proconnect_engagements_enriched
  // (client display fields, no tax IDs) or master_client_mapping instead.
  "proconnect_custom_statuses",
  "proconnect_engagements_enriched",
  "proconnect_sync_logs",
  "projects",
  "projects_enriched",
...
> = {
  ...
  proconnect_engagements_enriched: {
    description:
      "Live ProConnect Tax engagements (one row per tax return per year), enriched with client name/email, preparer, e-file status, and custom status. Ask this for 'where is X's 1040', return counts by preparer/year/status, or e-file progress.",
    key_columns: [
      "engagement_id",
      "proconnect_client_id",
      "tax_year",
      ...
      "preparer_name",
      "preparer_email",
      "preparer_team_member_id",
      "user_defined_status_name",
      "user_defined_status_color",
      "proconnect_created_at",
  ...
  proconnect_custom_statuses: {
    description:
      "Firm-defined workflow statuses configured in ProConnect Tax (name, color, ordering). Join user_defined_status_name on proconnect_engagements_enriched already resolves these.",
    key_columns: [
      "status_id",
      "name",
      "description",
  ...
  proconnect_sync_logs: {
    description:
      "Run history for the nightly ProConnect sync (status, counts, errors). Ask this to check whether tax data in the Hub is fresh.",
    key_columns: [
      "id",
      "sync_type",
      "status",
      ...
      "clients_synced",
      "engagements_synced",
      "custom_statuses_synced",
      "error_message",
    ],
  },
  tax_return_links_enriched: {
    description:
      "Cross-system view linking each ProConnect engagement to its Karbon work item and Ignition proposal/billing. Ask this for 'is X's return billed/worked', or tax-return project rollups.",
    key_columns: [
      "engagement_id",
      "proconnect_client_id",
      "tax_year",
  ...
  // -- Deals / pipeline / projects / intake -------------------------------
  deals: {
    description:
      "Sales opportunities (one deal per prospect). A deal sits ABOVE meetings and groups many meetings, debriefs, and tagged work items. stage = pipeline stage; status is open/won/lost. Prefer deals_enriched for read-only questions.",
    key_columns: [
      "id",
      "contact_id",
      "organization_id",
  ...
  deals_enriched: {
    description:
      "Read-only view: deals joined with contact/org/owner display names plus rollups (meeting_count, recorded_meeting_count, last_meeting_at, next_meeting_at, debrief_count, work_item_count). Use this for any 'what's in the pipeline' question.",
    key_columns: [
      "id",
      "title",
      "stage",
  ...
  projects: {
    description:
      "Hub-native projects that group Karbon work items by type/template pattern. status is active/paused/completed/archived. Prefer projects_enriched for read-only questions.",
    key_columns: [
      "id",
      "name",
      "kind",
  ...
  projects_enriched: {
    description:
      "Read-only view: projects joined with project type/template titles and a `clients` array of linked contacts/orgs. Use for 'what projects exist / what type / what status' questions.",
    key_columns: [
      "id",
      "name",
      "kind",
  ...
  prospect_submissions: {
    description:
      "Internal prospect intake form submissions (Hub-native, NOT the public Jotform). lead_status drives triage; push_to_* / *_push_status track sync to Karbon, ProConnect, and Ignition. Links to the master contact via contact_id once resolved.",
    key_columns: [
      "id",
      "submitter_full_name",
      "submitter_email",
      ...
      "referral_id",
      "karbon_push_status",
      "proconnect_push_status",
      "ignition_push_status",
      "created_at",
    ],
  },
  referrals: {
    description:
      "Referral relationships + the matching state machine. match_status resolves who referred a prospect. NEVER auto-create a contact for an unmatched row -- surface for human review only.",
    key_columns: [
      "id",
      "referee_contact_id",
      "referee_name",
  ...
  calendly_events: {
    description:
      "Raw Calendly scheduled events (source records behind `meetings`). Carries invitee counts, cancel/reschedule state, join_url, and the linked Zoom meeting via the /j/<id> in join_url. team_member_id is the Hub host.",
    key_columns: [
      "id",
      "name",
      "status",
  ...
  calendly_event_clients: {
    description:
      "Join table linking a Calendly event to Hub contacts/orgs. link_source is one of 'auto' | 'manual' | 'alfred' | 'calendly_bridge'. needs_review flags low-confidence ALFRED matches.",
    key_columns: [
      "id",
      "calendly_event_id",
      "contact_id",
  ...
  zoom_meetings: {
    description:
      "Raw Zoom meetings (source records behind `meetings`). calendly_event_id links back to the Calendly side via the deterministic bridge. Carries triage timestamps (alfred_triage_at, calendly_bridge_at) and start/end times.",
    key_columns: [
      "id",
      "zoom_meeting_id",
      "topic",
  ...
  zoom_meeting_clients: {
    description:
      "Join table linking a Zoom meeting to Hub contacts/orgs. link_source is 'auto' | 'manual' | 'alfred' | 'calendly_bridge'. needs_review flags low-confidence ALFRED matches.",
    key_columns: [
      "id",
      "zoom_meeting_id",
      "contact_id",
  ...
  zoom_meeting_work_items: {
    description: "Join table linking a Zoom meeting to Karbon work items.",
    key_columns: [
      "id",
      "zoom_meeting_id",
      "work_item_id",
  ...
  zoom_meeting_deals: {
    description: "Join table linking a Zoom meeting to a deal (sales opportunity).",
    key_columns: [
      "id",
      "zoom_meeting_id",
      "deal_id",
  ...
  zoom_meeting_projects: {
    description: "Join table linking a Zoom meeting to a Hub project.",
    key_columns: [
      "id",
      "zoom_meeting_id",
      "project_id",
