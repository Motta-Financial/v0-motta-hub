...
export async function researchProspectQuestions(
  ...
  let raw = ""
  // Widened to `string` so we can reassign from aiConfig.model
  let modelUsed: string = SUMMARY_MODEL
  try {
    // Fetch AI config for model override from the admin panel
    const aiConfig = await getAIConfig("question_research")
