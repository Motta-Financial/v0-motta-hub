...
export async function GET(request: Request) {
  try {
    ...
    // to reduce payload size. For normal requests, use work_items_enriched
    // view which pre-joins contacts, organizations, client_groups, and
    // team_members.
    ...
    // Build a fresh query per page. PostgREST caps any single response at
    // 1,000 rows regardless of the requested range, so limits above 1,000
    // are satisfied by looping .range() windows below.
    const buildQuery = () => {
      let query = supabase
    ...
    // Loop .range() windows (max 1,000 rows each) until the requested limit
    // is satisfied or a short page signals the result set is exhausted.
    const workItems: any[] = []
    let count: number | null = null
    let from = offset
    while (workItems.length < limit) {
      const chunkSize = Math.min(limit - workItems.length, 1000)
      const { data, error, count: chunkCount } = await buildQuery().range(from, from + chunkSize - 1)

      if (error) throw error

      if (count === null) count = chunkCount ?? null
