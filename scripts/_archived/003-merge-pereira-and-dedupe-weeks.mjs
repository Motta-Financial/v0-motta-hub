 ...
 *
 * Run with:
 *   node --env-file-if-exists=/vercel/share/.env.project scripts/003-merge-pereira-and-dedupe-weeks.mjs
 */
...
async function dedupeWeeks() {
  ...
  for (const [weekName, items] of Object.entries(groups)) {
    ...
    for (const dup of duplicates) {
      ...
      const { error: delErr } = await supabase.from("tommy_award_weeks").delete().eq("id", dup.id)
      if (delErr) {
